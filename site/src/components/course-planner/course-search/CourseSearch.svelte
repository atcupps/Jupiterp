...
<script lang="ts">
  ...
  function handleInput() {
    ...
    // alphabetical order
    searchResults = searchCourses(searchInput, courseLookup, deptList).sort(
      (a, b) => {
  ...
  $: {
    if (hoveredSection) {
      let index = searchResults.findIndex((course) => {
        return hoveredSection && course.code === hoveredSection.courseCode;
      });
      if (index === -1) {
  ...
  $: if (selections || hoveredSection) {
    totalCredits = 0;
    let selectionsWithHovered = appendHoveredSection(
      selections,
      hoveredSection
    );
...
{#if courseSearchSelected}
  <button
    class="fixed w-full bg-black bg-opacity-20 z-[51]
                    lg:hidden"
...
<!-- Course Search -->
<div
  class="lg:flex flex-col xl:min-w-[320px] 2xl:min-w-[400px] 2xl:text-lg
...
>
  <div
    class="flex flex-col w-full border-solid border-b-2 px-1 lg:px-0
  ...
  <div
    class="grow courses-list overflow-y-scroll overflow-x-none
