...
<div class="flex h-full w-full flex-col">
	...
	<div class="h-full min-h-0">
		<section
			class="schedule-list-pane min-h-0 shrink-0 overflow-y-auto rounded-xl
                border border-outlineLight bg-bgSecondaryLight
            p-2 dark:border-outlineDark dark:bg-bgSecondaryDark"
			style="width: {Math.max(180, Math.min(560, sidebarWidth))}px;"
			role="group"
			on:contextmenu={(event) => {
				openContextMenu(event, 'background', currentSchedule.term, currentSchedule.year);
			}}
		>
			{#each groups as group}
				<div
					class="mb-2 rounded-lg border border-divBorderLight
                            dark:border-divBorderDark"
				>
					<button
						class="flex w-full items-center justify-between px-2 py-1
                                    text-left text-sm
                                    font-semibold hover:bg-hoverLight dark:hover:bg-hoverDark"
					...
					{#if !collapsedGroupKeys.includes(group.key)}
						...
						>
							{#each group.rows as row}
								<div
									class="mb-1 rounded-md border border-outlineLight px-2
                                            py-1 hover:bg-hoverLight
                                            dark:border-outlineDark dark:hover:bg-hoverDark"
									role="group"
									...
									on:contextmenu={(event) => {
										openContextMenu(event, 'row', row.schedule.term, row.schedule.year, row.id);
									}}
								>
									{#if editRowId === row.id}
										<div class="flex flex-col gap-1">
											<input
												class="rounded border border-outlineLight bg-bgLight px-2
                                                            py-1 text-sm
                                                            outline-none dark:border-outlineDark dark:bg-bgDark"
												bind:value={editName}
											/>
											<div class="flex flex-row gap-1">
												<select
													class="rounded border border-outlineLight bg-bgLight px-1
                                                                py-1 text-xs
                                                                outline-none dark:border-outlineDark dark:bg-bgDark"
													bind:value={editTerm}
												>
												...
												<input
													class="w-20 rounded border border-outlineLight bg-bgLight
                                                                px-1 py-1
                                                                text-xs outline-none dark:border-outlineDark dark:bg-bgDark"
													type="number"
												...
												<input
													class="w-20 rounded border border-outlineLight bg-bgLight
                                                        px-1 py-1
                                                        text-xs opacity-75 outline-none dark:border-outlineDark dark:bg-bgDark"
													type="number"
											...
											<div class="flex flex-row gap-1">
												<button
													class="rounded border border-outlineLight px-2 py-1
									...
									{:else}
										<button class="w-full text-left" on:click={() => selectSchedule(row.id)}>
											<div class="truncate text-sm font-semibold">
												{row.schedule.scheduleName}
												{#if row.isCurrent}
													<span
														class="ml-1 rounded-full border border-outlineLight px-1.5
                                                                    py-0.5 text-[11px] dark:border-outlineDark"
														>Now viewing</span
													>
												{/if}
											</div>
											<div class="text-xs opacity-70">
												{row.schedule.term}
												{row.schedule.year} · Credits {rowCredits(row.schedule)}
											</div>
										</button>
	...
	{#if contextOpen}
		<div
			class="schedule-context-menu fixed z-[80] rounded-md border border-outlineLight
                    bg-bgLight py-1 text-sm
