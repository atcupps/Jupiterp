...
>
  ...
  {#if !meeting.hover}
    <div
      ...
      onclick={removeCourseByClassMeeting}
  ...
  {#if w >= 72}
    ...
    <div class="w-full grow px-2 font-sans text-xs font-thin 2xl:font-normal">
      ...
      {#if !meeting.userEvent && (h - 24 * fontSize > 32 * fontSize || isInOther)}
        <div class="static truncate">Section {secCode}</div>
      {/if}
      {#if h - 24 * fontSize > 16 * fontSize && hasLocation}
        <div class="static truncate" class:underline={meetingLocChange} class:decoration-dotted={meetingLocChange}>
          {#if meetingLocChange}
            <Tooltip text={'⚠ ' + location} tooltipText="Class location has changed since you last visited Jupiterp." />
          {:else}
            {location}
          {/if}
        </div>
