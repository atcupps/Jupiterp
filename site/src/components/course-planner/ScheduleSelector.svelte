...
<script lang="ts">
	...
	import { TERM_VALUES, MIN_SCHEDULE_YEAR, getMaxScheduleYear } from '$lib/course-planner/Terms';
	...
	let editTarget:
		| { source: 'current' }
		| { source: 'nonselected'; nonselectedIndex: number }
		| null = null;
	...
	function applyRows(rows: MutableRow[]) {
		...
		NonselectedScheduleStore.set(
			rows.filter((row) => row.source === 'nonselected').map((row) => row.schedule)
		);
...
<div class="flex w-full flex-col">
	{#if groups.length === 0}
		<div class="text-gray-500 px-2 py-1 text-sm">No schedules yet</div>
	{/if}
	...
	{#each groups as group}
		<section class="mb-2">
			<div class="flex items-center justify-between px-1 py-1">
				<button
					class="text-gray-500 inline-flex items-center gap-1 text-sm"
					type="button"
				...
				<button
					class="text-gray-600 inline-flex items-center gap-1 rounded px-2 py-1
                                text-xs hover:bg-hoverLight dark:hover:bg-hoverDark"
					type="button"
			...
			{#if !collapsedGroupKeys.includes(group.key)}
				...
				>
					{#each group.rows as row}
						<div
							class="relative flex items-center gap-2 rounded px-2 py-1"
							role="listitem"
						...
						>
							{#if dragOverRowIndex === row.displayIndex && draggedDisplayIndex !== row.displayIndex}
								<div class="absolute -top-0.5 left-1 right-1 h-0.5 bg-orange" />
							{/if}

							<button
								class="text-gray-700 grow truncate text-left text-sm"
							...
							<button
								class="rounded p-1 hover:bg-hoverLight dark:hover:bg-hoverDark"
								class:text-white={row.source === 'current'}
								type="button"
								title="Schedule actions"
								on:click|stopPropagation={(event) => {
									event.stopPropagation();
									openMenuRowKey = openMenuRowKey === row.rowKey ? null : row.rowKey;
								}}
							>
								⋯
							</button>
							...
							{#if openMenuRowKey === row.rowKey}
								<div
									class="absolute right-2 top-[calc(100%+2px)] z-20 min-w-28
                                            rounded-md border border-outlineLight bg-bgLight
                                        py-1 shadow-lg dark:border-outlineDark dark:bg-bgDark"
								>
									<button
										class="flex w-full items-center gap-1 px-2 py-1 text-left text-xs
...
>
	<form class="flex flex-col gap-3" on:submit|preventDefault={submitCreateSchedule}>
		<label class="text-sm">
			<span class="mb-1 block text-xs opacity-70">Schedule name</span>
			<input
				class="w-full rounded-md border border-outlineLight bg-bgLight
                            px-2 py-1 outline-none dark:border-outlineDark dark:bg-bgDark"
		...
		<div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
			<label class="text-sm">
				<span class="mb-1 block text-xs opacity-70">Term</span>
				<select
					class="w-full rounded-md border border-outlineLight bg-bgLight
                                px-2 py-1 outline-none dark:border-outlineDark dark:bg-bgDark"
			...
			<label class="text-sm">
				<span class="mb-1 block text-xs opacity-70">Year</span>
				<input
					class="w-full rounded-md border border-outlineLight bg-bgLight
                                px-2 py-1 outline-none dark:border-outlineDark dark:bg-bgDark"
					type="number"
