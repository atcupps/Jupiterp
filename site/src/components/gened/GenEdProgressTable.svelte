...
<script lang="ts">
	...
	const statusFilters: (GenEdStatus | 'all')[] = ['all', 'not_started', 'in_progress', 'completed'];
	...
	$: filteredRows = rows.filter((row) => {
		const categoryOk = selectedCategory === 'all' || row.requirement.category === selectedCategory;
...
<div class="flex w-full flex-col gap-3">
			...
			<label class="flex flex-col gap-1 text-xs">
				...
				<select
					class="rounded-md border border-outlineLight bg-bgLight
                           px-2 py-1 text-sm dark:border-outlineDark dark:bg-bgDark"
					bind:value={sortKey}
				>
	...
	<div class="w-full overflow-x-auto rounded-md border border-outlineLight dark:border-outlineDark">
		<table class="w-full min-w-[1050px] text-sm lg:min-w-full">
			<thead class="bg-hoverLight text-left dark:bg-hoverDark">
				<tr>
			...
			<tbody>
				...
				{#each sortedRows as row}
					<tr class="border-t border-outlineLight align-top dark:border-outlineDark">
						<td class="px-3 py-2">
						...
						<td class="whitespace-nowrap px-3 py-2">{row.requirement.category}</td>
						<td class="px-3 py-2">
							<span
								class={`inline-flex rounded-full px-2 py-0.5 text-xs ${statusClasses(row.status)}`}
							>
						...
						<td class="px-3 py-2">
							<button
								...
								on:click={() => {
									toggleExpanded(row.requirement.code);
								}}
							>
					...
					{#if expandedCodes.has(row.requirement.code)}
						<tr class="border-t border-outlineLight dark:border-outlineDark">
							<td colspan="8" class="px-3 py-2">
								{#if row.courses.length === 0}
									<div class="text-xs opacity-70">
										No completed courses tagged for this requirement yet.
									</div>
								{:else}
									<div class="grid grid-cols-1 gap-2 lg:hidden">
										{#each row.courses as course}
											<div
												class="rounded-md border border-outlineLight p-2 text-xs dark:border-outlineDark"
											>
												...
												<div>
													Status: {course.completionState === 'completed'
														? 'Completed'
														: 'In progress'}
												</div>
									...
									<table class="hidden min-w-full text-xs lg:table">
										<thead>
											<tr>
												<th class="py-1 text-left">Course</th>
												<th class="py-1 text-left">Term</th>
												<th class="py-1 text-left">Grade</th>
												<th class="py-1 text-left">Status</th>
												<th class="py-1 text-left">Other Gen Ed Tags</th>
											</tr>
										</thead>
										<tbody>
											{#each row.courses as course}
