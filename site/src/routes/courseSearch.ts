 ...
 * @returns A `Record` which can match department-name inputs to another `Record` matching
 *          course numbers to `Course` objects.
 */
export function getCourseLookup(departments: Department[]): Record<string, Record<string, Course>> {
...
/**
 * Given an `input`, search for courses in a `courseLookup` and return a list of possible `Course`s.
 * @param input A string of input used to search for `Course`s
 * @param courseLookup A `Record` used to match departments and course numbers to `Course`s;
 *                     can be generated using `getCourseLookup`
 * @returns An array of possible courses given the `input`.
 */
export function searchCourses(input: string, courseLookup: Record<string, Record<string, Course>>): Course[] {
    const result: Course[] = [];
