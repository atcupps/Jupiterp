<script lang="ts">
	...
	async function updateRequest(requestId: string, action: 'accept' | 'decline' | 'cancel') {
		if (!token) {
	...
	function friendLabel(friend: FriendRecord): string {
		return friend.friend.displayName ?? friend.friend.email ?? friend.friend.friendCode;
	}
...
>
	<div class="mx-auto flex max-w-5xl flex-col gap-4">
		...
		{#if loading}
			<div class="rounded-md border border-outlineLight p-3 dark:border-outlineDark">
				Loading friends...
			</div>
		{:else}
			<section class="rounded-md border border-outlineLight p-4 dark:border-outlineDark">
				<h2 class="mb-1 text-lg font-semibold">Add friends</h2>
				...
				<div class="mb-2 text-xs opacity-70">
					Your friend code: <span class="font-semibold tracking-wider">{selfFriendCode}</span>
				</div>
				<a
					class="mb-3 inline-flex rounded-md border border-outlineLight px-3 py-1 text-xs hover:bg-hoverLight
				...
				<div class="grid grid-cols-1 gap-3 md:grid-cols-2">
					<div class="rounded-md border border-divBorderLight p-3 dark:border-divBorderDark">
						<div class="mb-2 text-sm font-semibold">Add by email</div>
						<input
							class="mb-2 w-full rounded-md border border-outlineLight
                                    bg-bgLight px-2 py-1 outline-none dark:border-outlineDark dark:bg-bgDark"
							type="email"
							bind:value={emailInput}
							placeholder="friend@umd.edu"
						/>
						<button
							class="rounded-md border border-outlineLight px-3 py-1 text-sm hover:bg-hoverLight
                                    dark:border-outlineDark dark:hover:bg-hoverDark"
						...
						{#if emailFeedback}
							<div class="mt-2 text-xs opacity-80">{emailFeedback}</div>
						{/if}
					</div>

					<div class="rounded-md border border-divBorderLight p-3 dark:border-divBorderDark">
						<div class="mb-2 text-sm font-semibold">Add by friend code</div>
						<input
							class="mb-2 w-full rounded-md border border-outlineLight
                                    bg-bgLight px-2 py-1 uppercase outline-none dark:border-outlineDark dark:bg-bgDark"
						...
						{#if codeFeedback}
							<div class="mt-2 text-xs opacity-80">{codeFeedback}</div>
						{/if}
					</div>
				</div>
			</section>
			...
			<section class="rounded-md border border-outlineLight p-4 dark:border-outlineDark">
				<h2 class="mb-3 text-lg font-semibold">Friend requests</h2>
				<div class="grid grid-cols-1 gap-3 md:grid-cols-2">
					<div>
						<h3 class="mb-2 text-sm font-semibold">Incoming</h3>
						...
						{:else}
							<div class="flex flex-col gap-2">
								{#each incoming as request}
									<div
										class="rounded-md border border-divBorderLight p-2 dark:border-divBorderDark"
									>
										...
										<div class="mt-2 flex gap-2">
											<button
												class="rounded-md border border-outlineLight px-2 py-1 text-xs hover:bg-hoverLight
                                                        dark:border-outlineDark dark:hover:bg-hoverDark"
					...
					<div>
						<h3 class="mb-2 text-sm font-semibold">Outgoing</h3>
							...
							<div class="flex flex-col gap-2">
								{#each outgoing as request}
									...
									>
										...
										<button
											class="mt-2 rounded-md border border-outlineLight px-2 py-1 text-xs
			...
			<section class="rounded-md border border-outlineLight p-4 dark:border-outlineDark">
				<div class="mb-3 flex flex-wrap items-center justify-between gap-2">
					...
					<label class="flex items-center gap-2 text-sm">
						...
						<select
							class="rounded-md border border-outlineLight bg-bgLight px-2 py-1
							...
							<div class="rounded-md border border-divBorderLight p-3 dark:border-divBorderDark">
								<div class="flex flex-wrap items-center justify-between gap-2">
									...
									<div class="flex flex-wrap items-center gap-2">
										<select
											class="rounded-md border border-outlineLight bg-bgLight px-2 py-1 text-xs
                                                    dark:border-outlineDark dark:bg-bgDark"
