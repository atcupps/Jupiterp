...
import { sendFriendRequestByCode, sendFriendRequestByEmail } from '$lib/server/friends';
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		const message = toApiError(error instanceof Error ? error.message : 'Unable to send request');
