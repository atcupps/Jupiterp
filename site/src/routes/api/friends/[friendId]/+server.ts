...
import { removeFriend, updateFriendVisibility } from '$lib/server/friends';
...
export const PATCH: RequestHandler = async ({ request, params }) => {
	try {
		...
		const result = await updateFriendVisibility(supabase, userId, friendId, payload.visibility);

		return json(result, { status: result.ok ? 200 : 400 });
	} catch (error) {
		console.error('Update friend visibility error:', error);
		const message = toApiError(error instanceof Error ? error.message : 'Unable to update friend');
...
export const DELETE: RequestHandler = async ({ request, params }) => {
	try {
		...
		const message = toApiError(error instanceof Error ? error.message : 'Unable to remove friend');
		return json({ ok: false, message }, { status: 500 });
	}
};
