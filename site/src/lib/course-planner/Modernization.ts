...
function modernizeMeeting(legacy: LegacyClassMeeting): ClassMeeting {
	...
	if ('OnlineSync' in legacy) {
		...
		classtime = {
			...
			start:
				legacy.InPerson.classtime == null
					? 8
		...
		location = {
			building:
				legacy.InPerson.location == null || legacy.InPerson.location.length === 0
					? 'TBA'
					: legacy.InPerson.location[0],
			room:
				legacy.InPerson.location == null || legacy.InPerson.location.length < 2
					? null
					: legacy.InPerson.location[1]
		};
	}
...
function modernizeSection(courseCode: string, legacy: LegacySection): Section {
	return {
		courseCode,
...
export function modernizeSelections(selections: LegacyScheduleSelection[]): ScheduleSelection[] {
