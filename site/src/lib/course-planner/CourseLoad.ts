...
import { getDefaultTermYear, normalizeStoredSchedule } from './Terms';
...
function semesterFromTermYear(term: StoredSchedule['term'], year: number): string {
	if (term === 'Spring') {
		return `${year}01`;
	}
...
async function getUpToDateCoursesForSchedule(
	...
	const entries = await Promise.allSettled(
		Array.from(codes).map(async (courseCode) => {
...
export async function ensureUpToDateAndSetStores(
	...
	current.selections.forEach((selection) => {
		const upToDate: Course = upToDateCurrentCourses[selection.course.courseCode];
		if (!upToDate) {
