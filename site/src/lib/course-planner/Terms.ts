...
export function isAcademicTerm(value: unknown): value is AcademicTerm {
	return typeof value === 'string' && TERM_VALUES.includes(value as AcademicTerm);
}
...
export function normalizeStoredSchedule(
	...
	defaultValue = getDefaultTermYear()
): StoredSchedule {
	const maxYear = getMaxScheduleYear();
	const parsedYear = Number(schedule.year);
	const year =
		Number.isInteger(parsedYear) && parsedYear >= MIN_SCHEDULE_YEAR && parsedYear <= maxYear
			? parsedYear
			: defaultValue.year;

	const term = isAcademicTerm(schedule.term) ? schedule.term : defaultValue.term;
