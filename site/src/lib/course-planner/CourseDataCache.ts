...
export class CourseDataCache {
	...
	public async getCoursesAndSections(input: RequestInput): Promise<Course[]> {
		...
		this.pendingRequests[key] = this.requestCoursesForDept(key, requestConfig, !hadEntry, input);
		if (!hadEntry) {
			this.size += 1;
		}
...
function keyFromRequestInput(input: RequestInput): string {
	...
	const includeSectionsPart = input.includeSections === false ? '|sections:off' : '|sections:on';
...
function parseSectionRaw(raw: SectionRaw): any {
	const meetings =
		raw.meetings && raw.meetings.length > 0 ? raw.meetings.map(parseMeeting) : ['No Sections'];
