...
export async function gatherCoursesFromUmdIo(input: RequestInput): Promise<Course[]> {
	...
	return filtered.map((course) => {
		...
		return {
			...
			sections: includeSections ? (sectionsByCourse[course.course_id] ?? []).map(mapSection) : null
		};
	});
}
...
async function fetchCoursesByNumber(
...
function mapMeeting(rawMeeting: unknown): any {
	...
	const days = stringValue(meeting.days, meeting.day, meeting.weekdays, meeting['class_days']);
	const start = stringValue(meeting.start_time, meeting.start, meeting['meeting_start_time']);
	const end = stringValue(meeting.end_time, meeting.end, meeting['meeting_end_time']);
...
function normalizeGenEdCodes(rawGenEds: string[] | string | null | undefined): string[] {
	if (!rawGenEds) {
		return [];
	}
...
async function fetchJson<T>(path: string, query?: Record<string, string>): Promise<T> {
	...
	throw lastError instanceof Error ? lastError : new Error(`umd.io request failed for ${path}`);
}
