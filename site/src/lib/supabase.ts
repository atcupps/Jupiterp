...
export async function loadUserSchedules(userId: string): Promise<{
	currentSchedule: StoredSchedule;
	nonselectedSchedules: StoredSchedule[];
} | null> {
...
export async function ensureUserProfile(): Promise<UserProfileRow | null> {
	...
	const { data, error } = await supabase
		.from('profiles')
		.select(
			'id,email,display_name,friend_code,friends_visibility,degree_type,majors,minors,graduation_year,profile_privacy,avatar_url,avatar_color'
		)
		.eq('id', user.id)
...
export async function updateFriendsVisibility(value: FriendVisibility): Promise<void> {
	const user = await getAuthUser();
	if (!user) {
...
export async function updateProfilePreferences(preferences: ProfilePreferences): Promise<void> {
	const user = await getAuthUser();
	if (!user) {
		throw new Error('Sign in required');
	}
...
export async function uploadProfileAvatar(file: File): Promise<string> {
	...
	const { error: uploadError } = await storage.from('profile-avatars').upload(path, file, {
		cacheControl: '3600',
		upsert: true,
	...
	const { data } = storage.from('profile-avatars').getPublicUrl(path);
