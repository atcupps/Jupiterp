...
function normalizeCloudSchedules(cloudValue: {
...
export function readSchedulesFromLocalStorage(userId: string | null = null): StoredSchedule[] {
	...
	const scheduleTerm = isAcademicTerm(scheduleTermRaw) ? scheduleTermRaw : defaultTermYear.term;
	...
	const nonselected = nonselectedRaw ? resolveStoredSchedules(nonselectedRaw) : [];
...
export function schedulesToUserCourses(
	...
	for (const schedule of schedules) {
		...
		for (const selection of schedule.selections) {
			...
			const mergedGenEds = Array.from(new Set([...(existing.gen_ed_tags ?? []), ...nextGenEds]));
