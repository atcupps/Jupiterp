...
function compareNullableTerm(a: string | null, b: string | null, direction: SortDirection): number {
	if (a === null && b === null) {
		return 0;
	}
	...
	return [...rows].sort((left, right) => {
		...
		switch (sortKey) {
			case 'gen_ed_code':
				...
				break;
			case 'gen_ed_name':
				result = left.requirement.displayName.localeCompare(right.requirement.displayName);
				break;
			case 'category':
				result = left.requirement.category.localeCompare(right.requirement.category);
				break;
			case 'earliest_term':
				result = compareNullableTerm(left.earliestTermCode, right.earliestTermCode, direction);
				break;
			case 'latest_term':
				result = compareNullableTerm(left.latestTermCode, right.latestTermCode, direction);
				break;
			case 'status':
