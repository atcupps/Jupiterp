...
serve(async (request: Request): Promise<Response> => {
    ...
    try {
        ...
        const selfProfile = await ensureProfile(
            supabase,
            userId,
        ...
        if (request.method === 'GET' && endpoint === 'schedule') {
            ...
            const allSchedules = [
                ...
                ...schedules.nonselectedSchedules,
            ];

            const selectedSchedule = term && Number.isInteger(year)
        ...
        if (request.method === 'POST' && endpoint === 'requests') {
            ...
            const { data: existing, error: existingError } = await supabase
                .from('friends')
                .select('id,status')
                .or(`and(user_id.eq.${userId},friend_id.eq.${targetProfile.id}),and(user_id.eq.${targetProfile.id},friend_id.eq.${userId})`);
